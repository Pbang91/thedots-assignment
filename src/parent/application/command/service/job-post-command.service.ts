...
export class JobPostCommandService {
  ...
  public async createJobPost(
    dto: CreateJobPostData,
  ): Promise<CreateJobPostResDto> {
    const { addrDetail, sigungu, sido, zipcode, parentId, detail, lat, lng } =
      dto;
    ...
    await this.pushPort.sendJobPostCreated(teacherIds, {
      title: '새 공고가 등록되었습니다',
      body: `${sido} ${sigungu} 근처 공고를 확인해보세요`,
      data: { postId: savedPost.id },
    });
